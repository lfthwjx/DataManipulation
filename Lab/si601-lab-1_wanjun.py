# -*- coding: utf-8 -*-
"""
Created on Thu Sep  8 07:37:09 2016
...
def bananas(count):
    if count==0:
        ...
        return bananaLoop[0:len(bananaLoop)-2]
    else:
...
def match_ends(words, starting, ending):
    ...
    endwith=[]
    for word in words:
        if len(word)>6 and word.startswith(starting):
           stawith.append(word)
        if len(word)>6 and word.endswith(ending):
           endwith.append(word)
    staset = set(stawith)  
    endset = set(endwith)
