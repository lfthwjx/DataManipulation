...
with sqlite.connect(r'si-601-lab4.db') as con:
    ...
    cur.execute('''SELECT C_CUSTKEY,C_NAME,C_ACCTBAL FROM customer
    ...
    cur.execute('''SELECT C_MKTSEGMENT, SUM(C_ACCTBAL) AS T_ACCTBAL
                   FROM customer
                   GROUP BY C_MKTSEGMENT
                   ORDER BY T_ACCTBAL DESC ''')
    # fetchall() method gets all records. It returns a list of tuples.
    # Each of the tuples represent a row in the table.
    rows = cur.fetchall()
...
with sqlite.connect(r'si-601-lab4.db') as con:
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS nation
                   (N_NATIONKEY integer,
                    N_NAME varchar(100),
                    N_REGIONKEY integer,
                    C_COMMENT text)''')
    ...
    cur.execute('''SELECT N.N_NAME, SUM(C.C_ACCTBAL)
