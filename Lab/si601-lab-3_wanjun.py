...
def get_pigs():
    soup = BeautifulSoup(html_doc)
    pigs = soup.find_all('li')
    pig_tmp = []
    for pig in pigs:
        pig_tmp.append(str(pig.h2.string))
    json_tmp = json.dumps(pig_tmp)
    return str(json_tmp)
...
def get_story_headings():
    soup = BeautifulSoup(html_doc)
    result_str = []
    for story in soup(text=re.compile(r'Story')):
        result_str.append(str(story))
    json_tmp = json.dumps(result_str)
    return json_tmp
...
def get_houses():
    ...
    for tr in tr_result:
        tmp = []
        term = tr.find_all('td')
        tmp.append(str(term[0].string))
        tmp.append(str(term[1].string))
        results.append(tmp)
    results.remove(results[0])
    json_tmp = json.dumps(results)
    return str(json_tmp)
...
def treasure_hunting():
    html_open = urllib2.urlopen('http://www.example.com')
    soup = BeautifulSoup(html_open)
    link = soup.find('a').get('href')
    html_inside = urllib2.urlopen(link)
    soup_inside = BeautifulSoup(html_inside)
    content_rows = soup_inside.find_all('tr')
    result = content_rows[3].find('td').text
