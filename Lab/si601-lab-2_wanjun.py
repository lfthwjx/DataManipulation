...
def mission_report(s):
    pattern = r'(mission|raid)\s(\d+)\s.*?(failed|accomplished.*?\d+)'
    result = []
    matchs = re.findall(pattern, s, re.IGNORECASE)
    if len(matchs):
        for match in matchs:
            tmp = []
            tmp.append(int(match[1]))
            if 'failed' in match[2]:
                tmp.append('failed')
            elif 'accomplished' in match[2]:
                tmp.append(int(re.search(r'\d+', match[2]).group()))
