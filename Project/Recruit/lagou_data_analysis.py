...
cursor = db.cursor()
cursor.execute("DROP TABLE IF EXISTS lagou_data_analysis")
db.commit()
cursor.execute('''CREATE TABLE IF NOT EXISTS lagou_data_analysis(
...
def lagou_spider_keyword(keyword):
    ...
    i = 0
    type = 'true'
    url = 'http://www.lagou.com/jobs/positionAjax.json?px=default&first='+type+'&kd='+keyword+'&pn='+str(i+1)
    ...
    for i in range(totalPage):
        ...
        url = 'http://www.lagou.com/jobs/positionAjax.json?px=default&first=' + type + '&kd=' + keyword + '&pn=' + str(
            i + 1)
        data = urllib2.urlopen(url).read()
        data_json = json.loads(data)
        resultSize = int(data_json['content']['positionResult']['resultSize'])
    ...
    sql = """INSERT INTO lagou_data_analysis\
