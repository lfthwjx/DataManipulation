...
def get_post_fact(soupx, htmlx):
    ...
    descs = soup.findAll("li",{"class":"newlist_deatil_last"})
    for desc in descs:
        desc = desc.get_text()
        l_job_description.append(desc)
    ...
    save_to_sql(l_position_name, l_company_name, l_company_size, l_city, l_salary, l_education, l_job_description)


headers = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.94 Safari/537.36"}

urls = ["http://sou.zhaopin.com/jobs/searchresult.ashx?in=210500%3B160400%3B160000%3B160600&jl=%E9%80%89%E6%8B%A9%E5%9C%B0%E5%8C%BA&p={}&isadv=0".format(str(x)) for x in range(1, 89)]
...
cursor = db.cursor()
cursor.execute("DROP TABLE IF EXISTS zhaopin_it")
db.commit()
cursor.execute('''CREATE TABLE IF NOT EXISTS zhaopin_it(
...
def save_to_sql(l_position_name, l_company_name, l_company_size, l_city, l_salary, l_education, l_job_description):
    sql = """INSERT INTO zhaopin_it\
       SET position_name=%s,company_name=%s,company_size=%s,city=%s,salary=%s,education=%s,job_description=%s"""
    for x in range(0, len(l_position_name)):
        # print(len(l_post_name))
        # print(x)
        # print(l_fact_name)
        cursor.execute(sql,(l_position_name[x], l_company_name[x], l_company_size[x], l_city[x], l_salary[x], l_education[x], l_job_description[x]))
        db.commit()
